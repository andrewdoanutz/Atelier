...
export default class Account extends Component {
  ...
  componentDidMount(){
    axios.post('http://localhost:5000/api/db/getuser', {email: this.cookies.get("email")}).then(response => {
  ...
  saveAccountDetailsOnClick(){
    axios.put('http://localhost:5000/api/db/user', {email: this.state.email, password: this.state.password}).then(response => {
            console.log("SUCCESS", response)
            }).catch(error => {
                console.log(error)
            }).finally(() => {
